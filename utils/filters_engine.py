...
def should_forward(sms_data: Dict[str, Any], filters: Dict) -> bool:
    """
    Decide whether an SMS should be forwarded based on user filters.
    sms_data expected keys: from / sender, body / text / message
    """
    if not filters:
        return True

    sender = str(sms_data.get("from") or sms_data.get("sender") or "").strip()
    body = str(sms_data.get("body") or sms_data.get("text") or sms_data.get("message") or "")
...
def format_sms_message(device_id: str, sms_data: Dict[str, Any]) -> str:
    sender = sms_data.get("from") or sms_data.get("sender") or "Unknown"
    body = sms_data.get("body") or sms_data.get("text") or sms_data.get("message") or ""
    timestamp = sms_data.get("timestamp") or sms_data.get("receivedAt") or sms_data.get("time") or ""
