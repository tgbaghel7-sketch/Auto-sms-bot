...
WAITING_KEYWORD, WAITING_REGEX, WAITING_WHITELIST, WAITING_BLACKLIST = range(4)


async def filters_menu(update: Update, context: ContextTypes.DEFAULT_TYPE):
    await update.message.reply_text(
        "🔍 <b>Filters</b>\n\n"
        "Only SMS that pass your filters will be forwarded.\n"
...
async def filters_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data == "flt_add_kw":
        await query.edit_message_text(
            "Send a keyword (case-insensitive). SMS body must contain it.",
            reply_markup=back_main_keyboard(),
        )
        return WAITING_KEYWORD
    if data == "flt_add_re":
        await query.edit_message_text(
            "Send a regex pattern (e.g. \\d{6} for 6-digit OTP).",
            reply_markup=back_main_keyboard(),
        )
        return WAITING_REGEX
    if data == "flt_add_wl":
        await query.edit_message_text(
            "Send a sender number/name to whitelist.",
            reply_markup=back_main_keyboard(),
        )
        return WAITING_WHITELIST
    if data == "flt_add_bl":
        await query.edit_message_text(
            "Send a sender number/name to blacklist.",
            reply_markup=back_main_keyboard(),
        )
...
async def _add_filter(update: Update, key: str):
...
async def receive_keyword(update: Update, context: ContextTypes.DEFAULT_TYPE):
    return await _add_filter(update, "keywords")


async def receive_regex(update: Update, context: ContextTypes.DEFAULT_TYPE):
    return await _add_filter(update, "regex")


async def receive_whitelist(update: Update, context: ContextTypes.DEFAULT_TYPE):
    return await _add_filter(update, "whitelist")


async def receive_blacklist(update: Update, context: ContextTypes.DEFAULT_TYPE):
    return await _add_filter(update, "blacklist")
